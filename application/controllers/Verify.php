...
class Verify extends CB_Controller
{
	...
	public function confirmemail()
	{
		...
		if ( ! element('mae_id', $result)) {
			...
			$select = 'mem_id, mem_userid, mem_denied, mem_email_cert,mem_email';
	...
	public function resetpassword()
	{
		...
		if ( ! element('mae_id', $result)) {
			show_404();
			$view['view']['error_message'] = '잘못된 접근';
			$view['view']['error_message_detail'] = '잘못된 접근입니다';
		} elseif ( ! empty($result['mae_use_datetime']) && element('mae_use_datetime', $result) !== '0000-00-00 00:00:00') {
			$view['view']['error_message'] = '만료된 코드';
			$view['view']['error_message_detail'] = '회원님은 이미 패스워드 변경을 하셨습니다';			
		} elseif (strtotime(element('mae_generate_datetime', $result)) < ctimestamp()- 1800) {
			$view['view']['error_message'] = '만료된 코드';
			$view['view']['error_message_detail'] = '30분 이내에 변경 하셔야 합니다';
			...
			if ( ! element('mem_id', $dbmember)) {
				...
				$view['view']['error_message'] = '잘못된 접근';
				$view['view']['error_message_detail'] = '회원님의 계정은 접근이 금지되어 있습니다';
			} elseif ($this->cbconfig->item('use_register_email_auth') && ! element('mem_email_cert', $dbmember)) {
				$view['view']['error_message'] = '권한 없음';
				$view['view']['error_message_detail'] = '회원님은 회원가입 후, 또는 이메일 정보 변경후 아직 이메일 인증을 받지 않으셨습니다';
			}
			$view['view']['mem_userid'] = element('mem_userid', $dbmember);
			$view['view']['mem_email'] = element('mem_email', $dbmember);
		...
		if ($this->form_validation->run() === false) {
			...
			if (empty($view['view']['error_message'])) {
				...
				$view['view']['success_message'] = '새로운 비밀번호가 설정되었습니다.';				

				$this->member->update_login_log(element('mem_id', $result), element('mem_userid', $result), 1, '패스워드 변경 성공');
