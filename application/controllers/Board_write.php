	...
	public function _write_common($board, $origin = '', $reply = '')
	{
		...
		if ($form_validation === false OR $file_error) {
				...
				foreach ($form as $key => $value) {
					...
					if (element('field_type', $value) === 'text'
						...
						if (element('field_type', $value) === 'date') {
							...
							$extra_content[$k]['input'] .= '<input type="text" id="' . element('field_name', $value) . '" name="' . element('field_name', $value) . '" class="inp_txt js-input  js-inp-required" placeholder="숫자만 입력" value="' . set_value(element('field_name', $value)) . '" ' . $required . ' />';
						} else {
							$extra_content[$k]['input'] .= '<input type="' . element('field_type', $value) . '" id="' . element('field_name', $value) . '" name="' . element('field_name', $value) . '" class="inp_txt js-input  js-inp-required" value="' . set_value(element('field_name', $value)) . '" ' . $required . '/>';
						}
			...
			if(element('brd_key', $board) ==='b-a-9998')
				$redirecturl = write_url(element('brd_key', $board));
			else
